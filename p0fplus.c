/*
   ...
   Modified:
   30 November 2017 - John McKendry, CyGlass, Inc. (jfm) -
    in add_observation_field(), watch out for embedded quotes in value strings,
    because we are writing JSON and unescaped quotes will confuse the parser.
   4 December 2017 - jfm
    special-case code in expand_escapes() - do not insert an escape char
    before quote if there is already an escape char in the input string.
 */
...
*/
    ...
    time_t ut = get_unix_time_ms();
/*
    struct tm* lt = localtime(&ut);

    strftime((char*)tmp, 64, "%Y/%m/%d %H:%M:%S", lt);
    SAYF("[%s] mod=%s|cli=%s/%u|",tmp, keyword, addr_to_str(f->client->addr,
         f->client->ip_ver), f->cli_port);
*/
    SAYF("JSON:{\"timestamp\":%lld,\"mod\":\"%s\",\"cli\":\"%s\",\"cli_port\":%u,",ut, keyword, addr_to_str(f->client->addr,
         f->client->ip_ver), f->cli_port);

    SAYF("\"srv\":\"%s\",\"srv_port\":%u,\"subj\":\"%s\"", addr_to_str(f->server->addr, f->server->ip_ver),
...
/*
 * jfm 30 November 2017 - escape any quotes in the value string so
 * we don't break the JSON parser.
 */
...
*/
  ...
  if (value == (u8*)0){
	  ...
	  esc_value = malloc (2*strlen((char*)value) +1);
	  expand_escapes((char*)value, esc_value);
  }

  if (!daemon_mode) SAYF(",\"%s\":\"%s\"", key, esc_value ? esc_value : "???");

  if (log_file) LOGF("|%s=%s", key, esc_value ? esc_value : "???");
...
void expand_escapes(char* src, char* dest){
char c;
char *str_start = src;  // Don't look for a preceding character if
						// we're at the first character of the input.
 // jfm 30 Nov 2017 - this looks like parentheses overkill to me, but I
 //  get a compiler warning if I just say 'while (c = *(src++)) that
 //  politely suggests wrapping it in parentheses.
 while ((c = *(src++))) {
   switch(c) {
   case '\"':
	 // jfm 4 Dec 2017 - This is ugly, but every once in a while we
	 //  get a string here in which a quote has already been escaped -
	 //  specifically, this seems to happen with HTTP parameters from
	 // certain browsers. In those cases we don't want to insert another
	 // escape char.
	 // src has already been incremented to point to the next char in
	 // the input string, so we need to look back two characters, but not
	 // if we are dealing with the first char in the input string.
	 if ((src > str_start + 1) && (*(src-2) == '\\')){
		 *(dest++) = c;
		 break;
	 }
     *(dest++) = '\\';
     *(dest++) = '\"';
     break;
/*
 * jfm - I think the quote is the only character that confuses JSON,
 *  but let's keep some extra code for other special characters just in case.
   case '\a':
       *(dest++) = '\\';
       *(dest++) = 'a';
       break;
...
retry_no_vlan:
  ...
  if (!orig_rule) {
    ...
    if (vlan_support) {
      //final_rule = (u8*)"udp or tcp or (vlan and udp)  or (vlan and tcp)";
      final_rule = (u8*)"udp or tcp or (vlan and (udp or tcp))";
    } else {
      final_rule = (u8*)"udp or tcp";
    }
    ...
    if (vlan_support) {
      // jfm 3 Oct 2017
      final_rule = ck_alloc(strlen((char*)orig_rule) * 4 + 88);

      sprintf((char*)final_rule, "(udp and (%s)) or (tcp and (%s)) or (vlan and (udp and (%s) or tcp and (%s)))",
              orig_rule, orig_rule, orig_rule, orig_rule);

    } else {

      final_rule = ck_alloc(strlen((char*)orig_rule) + 40);

      sprintf((char*)final_rule, "(udp and (%s)) or (tcp and (%s))", orig_rule, orig_rule);
