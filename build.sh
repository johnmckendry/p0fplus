#!/bin/bash
#
# p0fplus - build script
# ------------------
#
# Copyright (C) 2017 by CyGlass, Inc. <support@cyglass.com>
#
# p0f original version Copyright (C) 2012 by Michal Zalewski <lcamtuf@coredump.cx>
#
# Distributed under the terms and conditions of GNU LGPL.
#
# Modified 16 Oct 2017 by John McKendry (jfm), CyGlass, Inc.
#  Added strutils.c and rtypes.c from the LANL dns_parse project
#   to OBJFILES.
#   As the project name suggests, these files contain functions for
#   parsing DNS Queries and Answers.
# Modified 6 December 2017 by John McKendry (jfm), CyGlass, Inc.
#  Changed PROGNAME to p0fplus and VERSION to 1.00.
#  p0f.c is now p0fplus.c .
#  Changed support contact information to "support@cyglass.com".
#  p0fplus is a derivative of p0f that CyGlass uses for data collection.
#  p0fplus does not change the fingerprinting functions that p0f performs.
#  It adds UDP packet header collection and some DNS payload analysis.
...
OBJFILES="api.c process.c fp_tcp.c fp_mtu.c fp_http.c readfp.c strutils.c rtypes.c"

echo "Welcome to the build script for $PROGNAME $VERSION!"
echo "Copyright (C) 2017 by CyGlass, Inc. <support@cyglass.com>"
echo
...
if [ ! -x "$TMP" ]; then
  ...
  cat "$TMP.log"
  echo
  echo "Sorry! You may want to ping <support@cyglass.com> about this."
  echo
  rm -f "$TMP.log"
  exit 1
...
echo "can reach the author at <support@cyglass.com>."
echo
